...
DOC_SESSION = _doc(
    ...
    '连接大约 10 分钟会换线（心跳无效）；无压缩时纯语音上下文约 15 分钟、开画面约 2 分钟。打字页没有单独分钟数，换线仍是 10 分钟。总表只在入门指南。',
)
DOC_VOICE = _doc(
    '语言与声音',
...
MODEL_RULES = [
    ...
    ['会话多久', '换线约 10 分钟（心跳无效）；无压缩：音频约 15 分钟 / 音视频约 2 分钟', '最多约 10 分钟', '流式连接可更长'],
...
SESSION_LIMITS = [
    ...
    [
        '三页都有 · 单根连接',
        '大约 10 分钟；提前约 60 秒 goAway',
        '前台接待、展台演示、听写字幕都会突然没声。不是网断了，也不是心跳没做好。客人往往要重新自我介绍，除非你们自动换线。',
        '勾选会话恢复，收到 goAway 后马上「接着上次」。官方允许反复换。句柄在 Vertex 大约 24 小时内还能用。',
        '开始并管理会话\nhttps://docs.cloud.google.com/gemini-enterprise-agent-platform/models/live-api/start-manage-session',
    ],
    [
        '对话页 · 只对着麦克风说',
        '不压缩时上下文大约 15 分钟；实际往往先撞上 10 分钟换线',
        '门店语音助手、车载问答：聊满一刻钟窗口会变慢或被掐。比换线宽一点，所以纯语音业务先按「每 10 分钟换一根线」设计。',
        '默认开会话恢复。接待经常超过一刻钟再开上下文压缩（旧话可能被忘掉）。',
        '开始并管理会话\nhttps://docs.cloud.google.com/gemini-enterprise-agent-platform/models/live-api/start-manage-session',
    ],
    [
        '对话 / 打字 · 开着摄像头或屏幕',
        '不压缩大约 2 分钟',
        '开箱、带看报错、举着手机讲解：大约两分钟画面会把窗口塞满。不是「只能上传一段 2 分钟录像」。',
        '只要画面开着就必须开上下文压缩。只换线、不压缩，两分钟仍会断。',
        '开始并管理会话\nhttps://docs.cloud.google.com/gemini-enterprise-agent-platform/models/live-api/start-manage-session',
    ],
    [
        '打字页 · 只打字、不开画面',
        '换线仍然大约 10 分钟。上下文涨得比「麦一直开」慢，官方没有单独给一个分钟数',
        '无麦演示、打字客服：不是「打字就能聊一整天」。线到点一样挂。你打一句它回一句，窗口涨得慢，但每句它念出来仍占额度。',
        '按 10 分钟换线来做。不要把对话页「纯音频 15 分钟」套到这一页。展台要开摄像头，按上面 2 分钟那条。',
        '开始并管理会话\nhttps://docs.cloud.google.com/gemini-enterprise-agent-platform/models/live-api/start-manage-session',
    ],
    [
        '听写页',
        '大约 10 分钟',
        '发布会字幕、现场记录：大约 10 分钟一切两断，不能当整场会的速记员。',
        '短场次够用。一小时会、会后出稿走 Speech-to-Text 批量。',
        'Gemini 3.5 Transcribe\nhttps://docs.cloud.google.com/gemini-enterprise-agent-platform/models/gemini/3-5-transcribe',
    ],
]
...
LIMIT_TALK = {
    'title': '这一页能撑多久（对业务）',
    'lede': '语音接待会在两处被掐：换线，和窗口。对照总表只在入门指南。',
    'items': [
        {
            'when': '客人一直对着麦说，不开摄像头',
            'impact': '大约 10 分钟会突然没声（换线限额）。不压缩的上下文大约还能再撑到 15 分钟，所以纯语音业务先卡在换线上。客人要重新自我介绍，除非自动「接着上次」。心跳救不了。',
            'do': '默认打开会话恢复。接待经常超过一刻钟再开上下文压缩。',
        },
        {
            'when': '开着摄像头或共享屏幕讲解',
            'impact': '不压缩大约 2 分钟就断。只够短导览、开箱，不够一路举着手机讲。不是「只能上传 2 分钟录像」。',
            'do': '画面开着就必须开压缩。只换线不够。',
        },
    ],
}
LIMIT_TRANSCRIBE = {
    'title': '这一页能撑多久（对业务）',
    'lede': '只要现场字幕。对照总表只在入门指南。',
    'items': [
        {
            'when': '对着麦克风出字幕',
            'impact': '大约 10 分钟一切两断。发布会、整场培训不能指望这一页当速记员。',
            'do': '短场次够用。一小时会走 Speech-to-Text 批量。需要接着记，勾选会话恢复并马上换线。',
        },
    ],
}
LIMIT_TEXTTURN = {
    'title': '这一页能撑多久（对业务）',
    'lede': '打字不是无限聊。对照总表只在入门指南。',
    'items': [
        {
            'when': '只打字、不开摄像头',
            'impact': '单根线仍然大约 10 分钟会挂，展台打字演示会突然没声。官方 15 分钟那条是「麦克风一直开着」的算法，不要套过来。你打一句它回一句，窗口涨得比对着麦慢，但每句它念出来仍占额度。',
            'do': '按 10 分钟换线做会话恢复。不要宣传「打字就能 24 小时在线」。',
        },
        {
            'when': '打字的同时开着摄像头或屏幕',
            'impact': '和对话页一样，不压缩大约 2 分钟。不能当「边打字边一直看屏幕讲解」。',
            'do': '画面开着就必须开压缩。',
        },
    ],
}
...
FEATURE_VALUE = [
    ...
    ['会话恢复', 'goAway 后点「接着上次」换一根 WebSocket；句柄约 24 小时有效', '突破约 10 分钟的单根连接上限（心跳解决不了）', '当无限长会；也不解决 128k 窗口装满', '三页都有'],
    ['上下文压缩', '滑动窗口丢掉旧轮次；本 Demo 约 5 万 token 触发、收到约 2.5 万', '突破无压缩时纯音频约 15 分钟 / 音视频约 2 分钟', '当完整会议记录；换线代替不了压缩', '三页都有'],
...
FEATURE_BLURBS = {
    ...
    'session_resume': {
        'effect': '服务会下发恢复句柄。大约 10 分钟单根 WebSocket 到点会 goAway，点「接着上次」把上下文接到下一根线。句柄大约 24 小时内有效。',
        'value': '突破 Google 给这根连接设的 10 分钟上限。心跳挡不住关线；网络闪断也能接着聊。',
        'caution': '解决的是换线，不是 128k 窗口。不压缩又开着画面，大约 2 分钟会先因上下文断掉。必须在点开始之前勾选。',
    },
    'compress': {
        'effect': '上下文接近上限时，服务端用滑动窗口丢掉或压缩旧轮次。本 Demo 约 50000 token 触发，收到约 25000。这样可以超过无压缩时的大约 15 分钟（纯音频）或 2 分钟（音视频）。',
        'value': '一直看着屏幕聊、接待超过两分钟。官方把压缩后的会话看成可拉得很长。',
        'caution': '解决的是 token 窗口，不是 10 分钟换线。较早的对话可能被忘掉。换线不会把已经装满的窗口变大。',
    },
    'affective': {
...
WORKSPACES = [
    {
        ...
        'limit_impact': LIMIT_TALK,
        'limit_note': '打断靠服务端 VAD；外放时请大声插话或戴耳机。摄像头是 JPEG 1fps，不是双向视频通话。不要在 SpeechConfig 里填普通话 locale。Vertex Live 同一会话只能选一种输出模态：这一页是 AUDIO。',
        'fit': [
